...
export async function saxpy(n, alpha, x, incx, y, incy) {
    ...
    if (!_pipeline) _pipeline = await loadShader("saxpy");

    const xBuffer = uploadBuffer(x, "saxpy-x", false);
    const yBuffer = uploadBuffer(y, "saxpy-y", true);
    ...
    const readBuffer = stageReadback(commandEncoder, yBuffer);
