...
import { createReadBuffer, prepareBindGroupEntries } from "./buffer.mjs";
...
export async function dispatch(
  ...
  if (Array.isArray(inPlaceBuffer)) {
    ...
    return await Promise.all(readBuffers.map((rb) => extractResult(rb, readbackType)));
  }
  ...
  device.queue.submit([commandEncoder.finish()]);
  return await extractResult(readBuffer, readbackType);
}
